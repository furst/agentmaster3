...
import { getObsidianConfig, getObsidianHooks, type ObsidianConfig } from '../core/project-config.js';
 ...
 * Uses registered hook if available, otherwise returns empty array (generic default)
 */
function inferTags(content: string, path: string): string[] {
	const hooks = getObsidianHooks();
	if (hooks.tagInference) {
		return hooks.tagInference(content, path);
	}
	// Generic default: no automatic tags
	return [];
}

/**
 * Generates frontmatter for a new note
 * Uses registered hook if available, otherwise generates minimal frontmatter
 */
function generateFrontmatter(tags: string[], type?: string): string {
	const hooks = getObsidianHooks();
	if (hooks.frontmatterGenerator) {
		return hooks.frontmatterGenerator(tags, type);
	}
	// Generic default: minimal frontmatter
...
export const writeVaultNoteTool = defineTool({
	...
	description:
		'Write or update a markdown note in the Obsidian vault. New notes get minimal frontmatter (tags can be provided or inferred via hooks). Includes safety measures: path validation, size limits, and automatic backups. Use mode="append" to add content to an existing note without overwriting.',
	parameters: z.object({
		path: z
			.string()
			.describe(
		...
		tags: z
			.array(z.string())
			.optional()
			.describe('Optional tags to add to frontmatter. If not provided, defaults to empty (unless tag inference hook is registered).'),
		type: z
			.string()
			.optional()
