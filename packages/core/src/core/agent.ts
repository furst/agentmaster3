...
/**
 * Serialized timeline entry for session persistence.
 * Tool calls have result/args serialized to handle any non-JSON-safe values.
 */
export interface SerializedTimelineEntry {
	id: string;
	type: 'user-message' | 'text-segment' | 'tool-call' | 'sub-agent';
	timestamp: number;
	content?: string;
	toolCall?: {
		id: string;
...
export interface AgentSession {
	...
	messages: CoreMessage[];
	stats: AgentStats;
	/** Timeline entries for UI restoration on resume */
