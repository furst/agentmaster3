	...
	switch (toolName) {
		...
		case 'web_search':
			if ('resultCount' in r) {
				...
				return `${r['resultCount']} results${modeStr}`;
			}
			break;
		case 'web_fetch':
			if ('contentCount' in r) {
				return `${r['contentCount']} pages`;
			}
			break;
		case 'web_research':
			if ('model' in r) {
		...
		case 'web_answer':
			if ('citationCount' in r) {
				return `${r['citationCount']} citations`;
			}
			break;
	}
	...
	for (const param of params) {
		...
		if (value !== undefined && value !== null) {
			if (typeof value === 'string') {
				...
				// For URL arrays (web_fetch, exa_get_contents), show domains
				if (param === 'urls' && value.every((v) => typeof v === 'string')) {
					const domains = value.map((url) => extractDomain(url as string));
					return truncate(domains.join(', '), 60);
				}
