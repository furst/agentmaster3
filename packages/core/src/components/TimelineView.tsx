...
/**
 * Check if a tool call is a sub-agent that should be hidden from the main timeline
 * These are shown via SubAgentEntry instead
 */
function isSubAgentToolCall(entry: TimelineEntry): boolean {
	...
	// Hide tools ending with _agent (legacy) and the task tool (new)
	return name.endsWith('_agent') || name === 'task';
}
