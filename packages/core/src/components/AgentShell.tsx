...
/**
 * Simple plan review for tool-initiated plans (just summary text)
 */
function ToolPlanReview({
	summary,
	onApprove,
	onCancel,
}: {
	...
	return (
		<Box flexDirection="column" marginTop={1}>
			...
			<Box flexDirection="column" marginLeft={2} marginTop={1}>
				...
				<Box marginTop={1}>
					<Text color="gray" dimColor>↑↓ navigate, Enter select, or A/C: </Text>
					{options.map((opt, i) => (
						<Box key={opt} marginLeft={1}>
							<Text color={selected === i ? (i === 0 ? 'green' : 'red') : 'gray'}>
								{selected === i ? '▸' : ' '}[{opt.charAt(0)}] {opt}
							</Text>
						</Box>
					))}
				</Box>
...
export function AgentShell({
	...
	const saveCurrentSession = useCallback(() => {
		...
		try {
			...
			// Include serialized timeline for UI restoration on resume
			session.timeline = getSerializedTimeline();
			saveSession(session);
		} catch (error) {
			// Silently fail - don't interrupt exit
			console.error('Failed to save session:', error);
		}
	}, [autoSave, getSerializedTimeline]);
	...
	const handleSessionSelect = useCallback((session: AgentSession) => {
		...
		// Import the selected session (restores messages and stats)
		agentRef.current.importSession(session);

		// Restore the timeline from the session (if available)
		if (session.timeline && session.timeline.length > 0) {
			importTimeline(session.timeline);
		} else {
			// Fallback: clear timeline if no saved timeline data
			reset();
		}
		...
		setHasStarted(true);
	}, [saveCurrentSession, importTimeline, reset]);
	...
	return (
		<Box flexDirection="column" padding={1}>
			...
			{/* Plan review section - user-initiated (structured plan) */}
			...
			{/* Plan review section - tool-initiated (summary only) */}
			{toolPlanSummary && !currentPlan && (
				<ToolPlanReview
					summary={toolPlanSummary}
					onApprove={handlePlanApprove}
					onCancel={handlePlanCancel}
				/>
			)}
