...
/**
 * Configuration schema for agentmaster CLI
 */
...
const CONFIG_DIR = join(homedir(), '.config', 'agentmaster');
...
export function getConfig(): Config {
	...
	if (process.env['AGENTMASTER_MODEL']) {
		envConfig.defaultModel = process.env['AGENTMASTER_MODEL'];
	}
...
export function getApiKey(): string {
	...
	if (!apiKey) {
		throw new Error(
			'Anthropic API key not configured. Set ANTHROPIC_API_KEY in a .env file, environment variable, or add anthropicApiKey to ~/.config/agentmaster/config.json'
		);
	}
